...
def cross_entropy(y_true, y_pred):
    skip = tf.not_equal(y_true, -1)
    skip = tf.reduce_min(tf.reduce_min(tf.to_int32(skip), 2), 1)
    skip = tf.to_float(skip)
    cr = tf.reduce_mean(
        -tf.reduce_sum(y_true * tf.log(y_pred), axis=2), axis=1)
    return skip * cr
...
def train_model(args):
    ...
    def data_provider(list_dat, aug, one_hot=True):
    ...
    for i, y in enumerate(Y):
        ...
        name = "dense_softmax_{}".format(datasets_batch_padding[i][0])
        net = softmaxPDF(n_outputs, n_classes, name=name)(Z)
        out.append(net)
